...
export default function DepositPage() {
  ...
  const submit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      console.log(e);
      toast({description: "Server is possibly down", title: "Error", className: "bg-red-600 text-white"});
    }
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-black text-white p-4">
      ...
      <form onSubmit={submit}>
        <div className="border-2 border-white rounded mt-4">
          <input type="file" name="file" className="p-2 text-white rounded w-64" onChange={e => setFile(e.target.files?.[0])} ref={ref} />
        </div>
        <div className="flex flex-row justify-center">
          <button type="submit" className="mt-4 bg-indigo-500 text-white py-2 px-3 rounded">
            Upload
            <span className="float-right">{isSubmitting &&
